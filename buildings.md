...
| **The order board** (at House 3) | Herlihy &amp; Wing 1990 | A board at House 3's gate on which the other houses' orders for its grain are entered and filled first come, first served: a FIFO queue. The same orders the tallies of *Ordering Without Clocks* order by chains of slips are here ordered by the sun: an order finished before another began is served first. *Before* is judged by the sun, which no one on Arche can read in the trading season; the goatherd of *Ordering Without Clocks* Ch. VI is the witness who could catch the houses out. The board has numbered **slots** for order tablets and one **peg** at the next free slot; clerks enter orders (take a number, put the tablet in) and loaders serve (go up from slot 1, take the first tablet) at once, with no one standing aside: the paper's §4 queue. Oil from House 2's presses has a board of its own, and all three houses order from both. Herlios keeps the board; Wingaia is of House 2. |
